...
function clamp(value: number, min = 0, max = 1): number {
  if (process.env.NODE_ENV !== 'production' && (value < min || value > max)) {
    console.error(
      `Kanban: The value provided ${value} is out of range [${min}, ${max}].`,
    );
  }
...
export function hslToRgb(color: string): string {
  const components = decomposeColor(color);
...
export function decomposeColor(
  color: string,
): { type: string; values: number[]; colorSpace?: string } {
  ...
  if (type === 'color') {
    ...
    if (valuesArray.length === 4 && values[3].charAt(0) === '/') {
      valuesArray[3] = valuesArray[3].substr(1);
    }
    if (
...
export function fade(color: string, value: number): string {
  ...
  if (components.type === 'rgb' || components.type === 'hsl') {
    components.type += 'a';
  }
  if (components.type === 'color') {
    // @ts-ignore
    components.values[3] = `/${value}`;
  } else {
    components.values[3] = value;
  }

  return recomposeColor(components);
}
